...
async function waitForRateLimit() {
  ...
  if (timeSinceLastRequest < MIN_DELAY_BETWEEN_REQUESTS) {
    ...
    console.log(
      `⏳ Rate limit protection: waiting ${waitTime}ms before next request`,
    );
...
function checkIdentityQuery(message) {
  ...
  if (
    isGenderQuery &&
    (msg.includes("creator") || msg.includes("dev") || msg.includes("you"))
  ) {
...
function addInitialWelcome() {
  if (chatMessages && chatMessages.children.length === 0) {
    const welcomeMessage =
      "✨ **Hey there! I'm IA Bot, your smart companion!** ✨\n\nI can:\n💬 Chat with you naturally\n📝 Set and save reminders\n🎯 Give motivation & study tips\n🧠 Remember our conversations\n\nTry saying: **'remind me to...'** or just ask me anything!\n\nWhat would you like to talk about? 😊";
    addMessage(welcomeMessage, false);
  }
}

// ===========================================
// REMINDER DETECTION - UPDATED WITH FUZZY TIME DETECTOR
// ===========================================
function detectReminder(message) {
  const reminderPatterns = [
    { pattern: /remind me to (.+?)(?: at | on | in | for |\.|$)/i },
    { pattern: /remind me (.+?)(?: at | on | in | for |\.|$)/i },
    { pattern: /don't forget to (.+?)(?: at | on | in | for |\.|$)/i },
    { pattern: /reminder:?\s*(.+?)(?: at | on | in | for |\.|$)/i },
    { pattern: /set a reminder for (.+?)(?: at | on | in | for |\.|$)/i },
    { pattern: /reminder:?\s*(.+?)(?: at | on | in | for |\.|$)/i },
  ];
  ...
  for (let pattern of reminderPatterns) {
    ...
    if (match && match[1]) {
      ...
      // Improved time pattern to catch 2pm, 200pm, 2:00pm
      let timePattern = /(\d{1,4})\s*([ap]m)/i;
      let timeMatch = message.match(timePattern);

      let formattedTime = null;
      if (timeMatch) {
        let rawNum = timeMatch[1];
        let ampm = timeMatch[2].toUpperCase();

        // Formatting logic
        if (rawNum.length <= 2) {
          formattedTime = `${rawNum}:00 ${ampm}`; // 2pm -> 2:00 PM
        } else {
          let mins = rawNum.slice(-2);
          let hours = rawNum.slice(0, -2);
          formattedTime = `${hours}:${mins} ${ampm}`; // 200pm -> 2:00 PM
        }
      }
      ...
      return {
        isReminder: true,
        text: task,
        time: formattedTime, // This will be null if no time was found
        original: message,
      };
    }
  }
...
function getFallbackResponse(message) {
  ...
  if (msg.match(/^(hi|hello|hey|yo|sup)/))
    return fallbacks.greetings[
      Math.floor(Math.random() * fallbacks.greetings.length)
    ];
  if (msg.match(/how are you|how're you|how you doing/))
    return fallbacks.howAreYou[
      Math.floor(Math.random() * fallbacks.howAreYou.length)
    ];
  if (msg.match(/thank|thanks|appreciate/))
    return fallbacks.thanks[
      Math.floor(Math.random() * fallbacks.thanks.length)
    ];
  if (msg.match(/bye|goodbye|see ya|peace/))
    return fallbacks.goodbye[
      Math.floor(Math.random() * fallbacks.goodbye.length)
    ];
  if (msg.match(/help|what can you do|what do you do/))
    return fallbacks.help[0];
  if (msg.match(/motivate|motivation|inspire/))
    return fallbacks.motivation[
      Math.floor(Math.random() * fallbacks.motivation.length)
    ];
  if (msg.match(/study|learn|review/))
  ...
  if (msg.match(/joke|funny|make me laugh/))
    return fallbacks.joke[Math.floor(Math.random() * fallbacks.joke.length)];
  if (msg.match(/stuck|confused|dont know/))
    return fallbacks.stuck[Math.floor(Math.random() * fallbacks.stuck.length)];
  if (msg.match(/procrastinate|later|mamaya|not now/))
    return fallbacks.procrastinate[
      Math.floor(Math.random() * fallbacks.procrastinate.length)
    ];
  if (msg.match(/tired|exhausted|pagod|drained/))
    return fallbacks.tired[Math.floor(Math.random() * fallbacks.tired.length)];
  if (msg.match(/nervous|anxious|scared|kinakabahan/))
    return fallbacks.nervous[
      Math.floor(Math.random() * fallbacks.nervous.length)
    ];
  if (msg.match(/done|finished|tapos/))
    return fallbacks.done[Math.floor(Math.random() * fallbacks.done.length)];

  // Check for reminders
  if (msg.match(/my reminders|show reminders|list reminders/)) {
    const userReminders = loadReminders(currentUserId);
    if (userReminders.length === 0)
      return "📭 You don't have any reminders yet bro! Want me to set one?";
    return `📋 Your reminders bro:\n${userReminders.map((r, i) => `${i + 1}. ${r.text}${r.time !== "later" ? ` (${r.time})` : ""}`).join("\n")}`;
  }

  // Default fallback
  return fallbacks.default[
    Math.floor(Math.random() * fallbacks.default.length)
  ];
}
  ...
  try {
    ...
    if (conversations[userId].length === 0) {
      contents.push({
        ...
        parts: [
          {
            text: "You are IA Bot, a friendly, helpful, and encouraging AI assistant. Give short, helpful responses with emojis. Keep responses under 3 sentences. Be warm and supportive. Use a casual, friendly tone like you're talking to a bro.",
          },
        ],
      });
      contents.push({
        ...
        parts: [
          {
            text: "Yo! 👋 I'm IA Bot, your personal assistant and friend. I'm here to chat, help with reminders, and keep you motivated. What's on your mind today, bro? 😊",
          },
        ],
      });
    }
    ...
    if (response.status === 429) {
      ...
      isRateLimited = true;
      if (status)
        status.textContent = "⏳ Rate limit reached. Waiting 10 seconds...";

      // Show rate limit message to user
      if (typing) typing.classList.remove("active");
      addMessage(
        "⏳ I'm getting a lot of requests right now. Please wait a moment before sending another message. 😊",
        false,
      );
...
function addMessage(message, isUser) {
  ...
  if (
    !isUser &&
    (window.ReactNativeWebView || window.webkit || window.parent !== window)
  ) {
...
// ===========================================
// SEND MESSAGE FUNCTION (WITH IDENTITY FIREWALL + MISSING TIME HANDLER)
// ===========================================
async function sendMessage() {
  ...
  if (isRateLimited) {
    addMessage(
      "⏳ Please wait a moment. I'm still recovering from too many requests. Try again in a few seconds. 😊",
      false,
    );
    chatInput.value = "";
    return;
  ...
  try {
    ...
    // 🔥 NEW LOGIC: Ask for time if missing
    if (reminderCheck.isReminder && !reminderCheck.time) {
      if (typing) typing.classList.remove("active");
      addMessage(
        `📝 I'll remind you to **${reminderCheck.text}**, but what time should I set it for, bro? (e.g., 2pm or 2:00pm)`,
        false,
      );
      if (status) status.textContent = "⏳ Waiting for time...";
      chatInput.style.height = "auto";
      if (chatInput) chatInput.focus();
      return; // Stop here and wait for user to type the time
    }

    // Offline reminder handling (existing logic with formatted time)
    if (reminderCheck.isReminder && !navigator.onLine) {
      saveReminder(currentUserId, reminderCheck.text, reminderCheck.time);
      if (typing) typing.classList.remove("active");
      addMessage(
        `🔔 Got it! I'll remind you to "${reminderCheck.text}"${reminderCheck.time ? ` at ${reminderCheck.time}` : ""}. I'll save this for when we're back online! 💪`,
        false,
      );
    ...
    if (status)
      status.textContent = GEMINI_API_KEY
        ? "✅ Gemini AI • Online"
        : "📡 Offline Mode";
  } catch (error) {
...
function init() {
  ...
  console.log(
    "🛡️ Identity Firewall Active - Creator queries will get instant responses!",
  );
}
